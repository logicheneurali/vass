...
- Reconnaissance vocale via Whisper (faster-whisper) avec noise floor adaptatif
- Synthèse vocale naturelle via Kokoro TTS avec chaîne de secours à 4 niveaux
- IA locale ou distante (llama.cpp, OpenAI, tout serveur compatible)
- Scripting VASScript pour automatisation bureau avec 25+ fonctions
- Gestion d'événements et planifications avec éditeur GUI
- Minuteur multilingue à commande vocale (5 simultanés)
- Serveur MCP avec 21 outils pour orchestration IA
- Mémoire permanente avec classification et résumé automatiques
- Visualiseur d'historique avec actions par message
- Support 9 langues
- Protection contre le débordement de contexte
- Sélection du périphérique audio (entrée/sortie)
- Appel d'outils multi-tour pour tâches IA complexes
...
### Installation graphique (recommandee)

Telechargez l'installeur depuis la [page Releases](https://github.com/logicheneurali/vass/releases) et executez-le. L'assistant installera Python, VASS, llama.cpp et un modele IA automatiquement — aucune configuration manuelle necessaire.
...
**Autorisation** : avant d'exécuter un script nouveau ou modifié, VASS affiche un popup demandant l'autorisation. Les scripts sont vérifiés par hachage SHA-256 : si un fichier script est modifié après avoir été autorisé, les permissions sont automatiquement révoquées et le popup réapparaîtra à la prochaine exécution. Cela garantit qu'aucun script ne peut s'exécuter sur votre machine sans votre consentement explicite.

Voir le fichier [VASCRIPT_REFERENCE.md](../Allowed_root/VASCRIPT_REFERENCE.md) pour la référence complète du langage.
