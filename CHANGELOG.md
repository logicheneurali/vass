...
Ultima compattazione: 2026-07-10, v0.6.5
...
### Calendario eventi
- **Giorno corrente**: triangolino rosso in basso a destra (non più bordo)
- **Giorno selezionato**: bordo bianco
- **Eventi ricorrenti futuri**: visualizzati nell'elenco come `↻ 10:00 — Descrizione [1d]`, corsivo, non selezionabili
- **Eventi disattivati**: pallini grigi sul calendario, testo giallo nell'elenco
- **Ordinamento cronologico** nell'elenco giornaliero
- **Validazione HH:MM e URL** nel settings editor al salvataggio

### find_free_slot
- **Nuovo tool MCP**: cerca il primo slot libero in una data, esclude pausa pranzo, rispetta eventi esistenti
- **Orario lavorativo configurabile** (08:00-19:00) e pausa pranzo (13:00-14:30) in `settings.ini`
- **Esclude eventi disattivati e include ricorrenze future**

### Fix critici (audit)
- `decrypt_fields()` ora ritorna l'output (prima `None`)
- Health check loop ora aggiorna GUI periodicamente (non solo allo shutdown)
- `VASScript._state` protetto da lock (thread-safe)
- `enabled`/`silent` JSON booleani gestiti con `str()` (AttributeError fix)
- `QSpinBox` import mancante in settings_editor
- `QMenu` import mancante in settings_editor
- TTS esterno (email, eventi) differito quando AI occupata
- `find_free_slot` ACL indentato correttamente in tools.yaml
- I18n work/lunch keys spostate da `field_descriptions` a `field_labels`

### File Chiave
| File | Linee | Contenuto |
|------|-------|-----------|
| `src/events_editor.py` | 732 | Calendario, pallini attivi/inattivi, ricorrenze future |
| `mcp_server/src/mcpgoal/server.py` | 486 | `find_free_slot`, `_find_free_slot` con settings.ini |
| `src/settings_editor.py` | 798 | Validazione HH:MM/URL/interi, orario lavorativo |
| `src/tts_engine.py` | 556 | `defer_if_busy`, `_flush_deferred` |
| `src/utils.py` | 514 | `generate_recurrences()`, `decrypt_fields` fix |
| `VERSION` | 1 | Bump a 0.6.5 |
